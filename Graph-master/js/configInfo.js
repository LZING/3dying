...
(function(self){
	...
	self.getCategoryData = function(name){
		return mmdb.ci.category.get(name);
	};
	
	self.getCategory = function(){
		return mmdb.ci.category.getAllByStructure();
	};
	...
	self.openChild = function(neo4jId, rs, term, dir){
		...
		result = mmdb.relation.ciinformation.queryCiRelationById(neo4jId, rs, term, dir);
			...
			try {
				for(i=0; i<result.node.length; i++){
					if(!self.editor.graph.getModel().getCell(result.node[i]._neo4jid_)){
						img = graph.onlineEdit.icon[result.node[i]._categoryId_];
						self.createCell(
							self.editor.graph,
							parent,
	...
	self.getListData = function(){
		...
		result = mmdb.ci.category.getAllByStructure();
	...
	self.initSelect = function(list){
		...
		update = function(name){
			...
			result = mmdb.ci.category.get(name);
			bSelect.html("");
		...
		self.container.find("input[type=button]").click(function(){
			...
			result = mmdb.ci.information.qureyByAdvanced(cateid, where, 1,  self.pageSize);
	...
	self.writeTable = function(arr){
		...
		if(arr.length > 0){
			...
			html = graph.utils.render("configInfo/table.html", {
				list: arr
			});
		...
		if(count){
			...
			for(i=1; i<=total; i++){
				...
				a.onclick = function(){
					...
					result = mmdb.ci.information.qureyByAdvanced(
	...
	self.createCell = function(g, parent, id, value, x, y, img){
		var width = "",
			height = "",
			style = "",
			image = "";
		...
		image = img ? img : "../" + getLibItem(getSelectCache("cateid")).ico;
	...
	self.createDropHandler = function(sCell, allowSplit) {
		return function(graph, evt, target, x, y) {
			...
			(function(list){
				...
				try {
					for(i=0; i<list.length; i++) {
						...
						if(!getObj(id)){
							cell = self.createCell(graph, parent, id, name, x, y);
							ids.push(id);
							graph.refresh(cell);
						}
					}
			...
			for(i=0; i<ids.length; ids++){
				...
				result = mmdb.relation.ciinformation.queryCiRelationById(id, null, null, {down:9, up: 9});
	...
	self.init = function(){
		...
		self.containerTable = self.container.find(".configinfo-table");
		self.containerPage = self.container.find(".configinfo-page");
...
})(nameSpace.reg("graph.configInfo"));
