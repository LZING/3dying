...
(function(self){
	...
	initTable = function(data){
		...
		html = graph.utils.render("share/simpleTable.html", {
			list: data
		});
		...
		$table.find("tr").each(function(){
			...
			$(_this).find("button").click(function(){
				var o = {};
				o.cate = $(_this).find("td:eq(0)").html();
				o.name = $(_this).find("td:eq(1)").html();
				o.id = $(_this).attr("nid");
				addItem(o);
			});
		});
	...
	initSearch = function(){
		var handle = function(){
			...
			result =  mmdb.ci.information.qureyByAdvanced(cateid, where, 1,  100000);
