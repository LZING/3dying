...
(function(){
	...
	// 更新图标
	var updateIco = function(element, defaultOptions){
		$(element).find(".ico").each(function(){
			mmdb.mxgraph.sidebar.addImagePalette(
				this,
				$(this).attr("name"),
				$(this).attr("ico"),
	...
	T.fn.createMenu = function(element, options){
		...
		if($(element).find(".jsontree").length > 0){
			element = $(element).find(".jsontree").get(0);
		}else{
		...
		switch(defaultOptions.layout){
			case "tree":
				(function(){
					...
					crehtmlFun = function(o){
						var i, html = [], childrenNode = "";
						
						for(i in o){
							childrenNode = "";
							if(typeof o[i].childrenNode === "object"){
								childrenNode = crehtmlFun(o[i].childrenNode);
							}
							
							html.push( _this.template(tpl,{
								name : o[i].name,
								ico : typeof o[i].image === "string" ? o[i].image : defaultOptions.ico ,
			...
			case "box":
				(function(){
					var crehtmlFun, item, treeToArr, html = "", arr = [];
					
					crehtmlFun = function(o){
						var i, n, html = [], childrenNode;
							
						for(i in o){
							
							treeToArr(o[i].childrenNode);
							arr.unshift({
								name : o[i].name,
								ico : typeof o[i].image === "string" ? o[i].image : defaultOptions.ico
							});
...
(function(){
	...
	// 目录
	path = "./javascripts/tree/templates",
