...
class IndexAction extends BaseAction{
   ...
   public function defaultAction() {
	   	if (!$this->checkLogin() || !$_SESSION['user']['admin']) {
	   		HttpNamespace::redirect('/admin/index/login/');
	   	}else {
	   		$this->display();
	   	}
   ...
   public function loginAction() {
   	...
   	if(HttpNamespace::isPost()) {
   		...
   		if ($msg['code']==1){
   			...
   			HttpNamespace::redirect('/admin/index/');
   		}
   	}else {
   		$this->display("/admin/login.php");
   	}
   ...
   public function repwdAction() {
   		...
   		if ($user){
   			...
   			if (HttpNamespace::isPost()) {
   				...
   				if ($data == null){
   					...
   					if(UserNamespace::updateUserInfo($info, $uid)) {
   						HttpNamespace::redirect('/admin/index/user/',3,"修改成功",'target="content"');
   					}
   				}else {
   					$this->assign($data);
   				}
   			...
   			HttpNamespace::redirect('/admin/index/user/');
   		}
   ...
   public function unupgradeAction() {
    ...
    public function attaAction() {
    	...
    	$this->assign('page',$page);
    	$this->display("/admin/index_atta.php");
    }
    ...
    public function imageAction(){
        if (HttpNamespace::isPost()) {
            ...
            HttpNamespace::redirect('/admin/index/image');
        }
        $this->display();
    }
