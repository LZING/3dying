...
* 2g以上超大文件也有效
* 几个大问题：
*1、没有关闭输出压缩、输出缓冲（自动缓冲）和magic_quotes（看手册评论）；
**3、ob_flush使用有缺陷，缓冲区未打开，未结束。单纯使用ob_flush会产生警告。
4、RANGE获取的格式不全，参考rtf文档:http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.35。
5、输出前清理已有缓冲内容也是有必要的。
6、flush 和  ob_flush是反的，且输出缓冲可能无效（正在验证）。
* @author MoXie
*/
class Transfer {
    /**
    ...
    public function send() {
        ...
        if ($fileHande) {
            ...
            //set_magic_quotes_runtime(0);有问题此处
