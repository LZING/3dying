...
THREE.NormalControls.prototype.addEventListeners = function() {
  ...
  this.element.addEventListener('mousedown', function(event){
    ...
    //console.log('x='+self.camera.position.x+'---y='+self.camera.position.y+'---z='+self.camera.position.z);
  }, false);
...
THREE.NormalControls.prototype.updateMousePos = function(event) {
  ...
  if (event.touches !== undefined && event.touches.length == 1 ) {
	  this.prevMousePos[0] = event.touches[ 0 ].pageX;
	  this.prevMousePos[1] = event.touches[ 0 ].pageY;
  } else {
...
THREE.NormalControls.prototype.onMouseMove = function(event) {
	...
	if (event.touches !== undefined) {
		mouseMotion = [event.touches[ 0 ].pageX-this.prevMousePos[0], event.touches[ 0 ].pageY-this.prevMousePos[1]];
	} else {
		mouseMotion = [event.clientX-this.prevMousePos[0], event.clientY-this.prevMousePos[1]];
		
	}
   
  
  if (this.mouseButton[10]) {
	// rotate around the target
	var scaleFactor = -.005;
	this.rotateAroundTarget(mouseMotion[0]*scaleFactor, mouseMotion[1]*scaleFactor);
  } else if ( this.mouseButton[0]&&this.keyboard[82] )// 'r' key
  {
    // this is how we roll
    var rotation = new THREE.Matrix4();
