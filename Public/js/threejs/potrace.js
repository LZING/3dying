...
var Potrace = (function() {
  ...
  function loadCanvas() {
	  if (info.maxSize > 0) {
		  var r = imgElement.width / imgElement.height;
		    var max = Math.min(Math.max(imgElement.width, imgElement.height), info.maxSize);
		    var rWidth = r >= 1 ? max : max*r;
		    var rHeight = r >= 1 ? max/r : max;
		    ...
		    ctx.drawImage(imgElement, 0, 0, rWidth, rHeight);
	  } else {
    ...
    //document.getElementById('imgElement').src = imgCanvas.toDataURL("image/png");
  }
  ...
  function loadBm() {
    ...
    for (i = 0, j = 0; i < l; i += 4, j++) {
      ...
      if (imgdataobj.data[i + 3] == 0) color = 255;
      if (!info.bmgrayNO) {
    	  bm.data[j] = (color < info.bmgray ? 1 : 0);
      } else {
    	  bm.data[j] = (color < info.bmgray ? 0 : 1);
      }
  ...
  function getPolygons(size, divisions) {
  ...
  var subj_paths = [];
    ...
    function path(curve) {
      ...
      function bezier(i) {
        shape.bezierCurveTo ( curve.c[i * 3 + 0].x * size, curve.c[i * 3 + 0].y * size, curve.c[i * 3 + 1].x * size, curve.c[i * 3 + 1].y * size, curve.c[i * 3 + 2].x * size, curve.c[i * 3 + 2].y * size );
      }
    
      function segment(i) {
        shape.lineTo(curve.c[i * 3 + 1].x * size, curve.c[i * 3 + 1].y * size);
        shape.lineTo(curve.c[i * 3 + 2].x * size, curve.c[i * 3 + 2].y * size);
      }

      var n = curve.n, i;
      shape.moveTo(curve.c[(n - 1) * 3 + 2].x * size, curve.c[(n - 1) * 3 + 2].y * size);
      for (i = 0; i < n; i++) {
        if (curve.tag[i] === "CURVE") {
          bezier(i);
        } else if (curve.tag[i] === "CORNER") {
      ...
      polygons.push(shape.extractAllPoints(divisions).shape);      
    }

    var w = bm.w * size, h = bm.h * size, len = pathlist.length, c, i;
    ...
    for (i = 0; i < len; i++) {
      ...
      path(c);
       ...
       for(var j=0;j<polygons[i].length;j++){
               onePath.push({X:polygons[i][j].x,Y:polygons[i][j].y});
               tempPath.push(polygons[i][j].x,polygons[i][j].y);
       }
       subj_paths.push(onePath);
       temp_paths.push(tempPath);
            
    }

    //console.log(JSON.stringify(subj_paths));
    //console.log(JSON.stringify(temp_paths));    
    return subj_paths;
    //
//    var co = new ClipperLib.ClipperOffset(2.0, 0.25);
//    co.AddPaths(subj_paths, ClipperLib.JoinType.jtMiter, ClipperLib.EndType.etClosedPolygon);
//    co.Execute(subj_paths, -0.5);
//    
...
//            var shape = new THREE.Shape();
//            shape.moveTo( solution_exPolygons[i].outer[0].X, solution_exPolygons[i].outer[0].Y );
//            for(j=1;j<solution_exPolygons[i].outer.length;j++){
//                    shape.lineTo( solution_exPolygons[i].outer[j].X, solution_exPolygons[i].outer[j].Y );
//            }
//            shape.lineTo( solution_exPolygons[i].outer[0].X, solution_exPolygons[i].outer[0].Y );
//            for(j=0;j<solution_exPolygons[i].holes.length;j++){
//                    var holePath = new THREE.Path();
//                    holePath.moveTo( solution_exPolygons[i].holes[j][0].X, solution_exPolygons[i].holes[j][0].Y );
//                    for(jj=1;jj<solution_exPolygons[i].holes[j].length;jj++){
//                            holePath.lineTo( solution_exPolygons[i].holes[j][jj].X, solution_exPolygons[i].holes[j][jj].Y );
//                    }
//                    holePath.lineTo( solution_exPolygons[i].holes[j][0].X, solution_exPolygons[i].holes[j][0].Y );
//                    shape.holes.push( holePath );
//            }
//            var geometry = new THREE.ExtrudeGeometry( shape, extrudeSettings );
//            var mesh = new THREE.Mesh( geometry, new THREE.MeshPhongMaterial( { color: 0x0000ff,specular: 0xA0A0A0, shading: THREE.SmoothShading, shininess: 150, fog: false, side: THREE.DoubleSide } ) );
//            group.add(mesh);
//    };
//    group.rotation.set( 0, Math.PI, Math.PI );
//    group.scale.set( 0.25, 0.25, 0.25 );
//    group.position.set(-70, 40, 2);
//    return group;
  }
